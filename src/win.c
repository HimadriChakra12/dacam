...
/* wl_keyboard sends raw Linux evdev keycodes (key + 8 = evdev; but
 * wl_keyboard already sends key - 8 removed, i.e. the raw evdev code
...
static void kbd_noop_leave(void *d, struct wl_keyboard *k, uint32_t s,
                            struct wl_surface *sf)
	{ (void)d;(void)k;(void)s;(void)sf; }
static void kbd_noop_modifiers(void *d, struct wl_keyboard *k, uint32_t s,
                                uint32_t mk, uint32_t ml, uint32_t ml2,
                                uint32_t mg)
	{ (void)d;(void)k;(void)s;(void)mk;(void)ml;(void)ml2;(void)mg; }
static void kbd_noop_keymap(void *d, struct wl_keyboard *k, uint32_t fmt,
                             int fd, uint32_t size)
	{ (void)d;(void)k;(void)fmt; close(fd);(void)size; }
static void kbd_noop_repeat(void *d, struct wl_keyboard *k, int32_t rate,
                             int32_t delay)
	{ (void)d;(void)k;(void)rate;(void)delay; }
static const struct wl_keyboard_listener keyboard_listener = {
	.keymap      = kbd_noop_keymap,
	.enter       = kbd_noop_enter,
	.leave       = kbd_noop_leave,
	.key         = kbd_key,
	.modifiers   = kbd_noop_modifiers,
	.repeat_info = kbd_noop_repeat,
};
